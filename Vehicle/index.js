import { Vehicle } from '../../declarations/Vehicle';
...
regBtn.addEventListener('click', async (e) => {
  ...
  if (imsId.length === 0) {
    ...
    progressBar.play();
    try {
      await Vehicle.setRcNo(rcNo);
      await Vehicle.setImsAddr(imsId);
      await Vehicle.registerVehicle();
      alert("Vehicle registeration Successful! \nThis portal will now  be transformed into user's Vehicle dashboard");
      var rcNoFromCan = await Vehicle.getRcNo();
      registerGroup.style.display = 'none';
      vehicleDashboardGroup.style.display = 'block';
      progressBar.stop();
      initilizeMap();
...
document.querySelector('.btn-rc-disp').addEventListener('click', async () => {
  var copyText = await Vehicle.getVid();
...
function initilizeMap() {
  ...
  window.addEventListener('load', async () => {
    await Vehicle.resetAlert();
    await Vehicle.resetWarning();
  });
...
startBtn.addEventListener('click', async () => {
  if (isRunning) {
    ...
    await Vehicle.resetAlert();
    await Vehicle.resetWarning();
    await Vehicle.updateVehicleInfo(0, mLat, mLng);
    ...
    listener = setInterval(async () => {
      try {
        await Vehicle.updateVehicleInfo(parseInt(speedRange.value), mLat, mLng);
        console.log(parseInt(speedRange.value));
        var x = await Vehicle.getSL();
        console.log(x);


        if (await Vehicle.alertListener()) {
          sendAlert();
          await Vehicle.resetAlert();
          // cooldown++;
        }
        else if (await Vehicle.warningListener()) {
          sendWarning();
          await Vehicle.resetWarning();
          // cooldown++;
        }
